...
let
  ...
  # Helper to get config filename for each app
  ...
  # Helper to get reload method for each app
  # Returns: { method = "touch" | "signal" | "command"; ... }
  getAppReloadMethod = app: {
    alacritty = {
      method = "touch";
      # Touch the symlink to trigger Alacritty's file watcher
      # Use -h to touch symlink itself, not target (which is in read-only /nix/store)
    };
    btop = {
      method = "touch";
    };
    ...
    shell-colors = {
      method = "none";  # Sourced in shell, takes effect on new shell sessions
    };
    ls-colors = {
      method = "none";  # Sourced in shell, takes effect immediately
    };
    console = {
      method = "command";
  ...
  themeVariantPackages = mapAttrs (themeName: theme:
    mapAttrs (variant: baseColors:
      ...
      pkgs.runCommand "vogix16-theme-${variantName}" {} ''
        ...
        ${concatMapStringsSep "\n" (app:
          let
            generator = appGenerators.${app} or null;
            configFileName = getConfigFilename app;
            configDir = dirOf configFileName;
          in
          optionalString (generator != null) (
            if app == "console" then
              # Console palette: strict format, no comments (setvtrgb expects exactly 16 hex lines)
              ''
...
EOF
              ...
              # Other configs: add metadata header
              ''
                mkdir -p "$out/${app}/${configDir}"
                cat > "$out/${app}/${configFileName}" <<'EOF'
...
${concatMapStringsSep "\n\n" (app:
  let
    configFileName = getConfigFilename app;
    reloadMethod = getAppReloadMethod app;
...
MANIFEST_EOF
    ...
    # NOTE: User must enable bash themselves (programs.bash.enable = true)
    programs.bash = mkIf (builtins.elem "shell-colors" themedApps || builtins.elem "ls-colors" themedApps) {
      # Source in both .bashrc (interactive shells) and .bash_profile (login shells)
      initExtra = mkIf (builtins.elem "shell-colors" themedApps || builtins.elem "ls-colors" themedApps) ''
        ${optionalString (builtins.elem "shell-colors" themedApps) ''
          if [ -f ~/.config/shell-colors/colors.sh ]; then
            source ~/.config/shell-colors/colors.sh
          fi
        ''}
        ${optionalString (builtins.elem "ls-colors" themedApps) ''
          if [ -f ~/.config/ls-colors/dircolors ]; then
            source ~/.config/ls-colors/dircolors
          fi
        ''}
      '';
      profileExtra = mkIf (builtins.elem "shell-colors" themedApps || builtins.elem "ls-colors" themedApps) ''
        ${optionalString (builtins.elem "shell-colors" themedApps) ''
          if [ -f ~/.config/shell-colors/colors.sh ]; then
            source ~/.config/shell-colors/colors.sh
          fi
